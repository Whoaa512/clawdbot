...
export async function runCronIsolatedAgentTurn(params: {
	...
	if (shouldWrapExternal) {
		...
		const { prompt: safeContent, analysis } = buildSafeExternalPrompt({
		...
		// Log analysis findings for security monitoring
		if (!analysis.clean) {
			logWarn(
				`[security] Content analysis: ${analysis.riskLevel} (score=${(analysis.score * 100).toFixed(0)}%, ` +
					`findings=${analysis.findings.length}, session=${baseSessionKey})`,
			);
		}
